...
class AuthSessionStorage {
    ...
    private let sessionKey = "com.neatlify.auth.session"
    private let refreshTokenKey = "com.neatlify.auth.refreshToken"
    private let userIdKey = "com.neatlify.auth.userId"
    private let userEmailKey = "com.neatlify.auth.email"

    private init() {}

    func saveSession(_ session: AuthenticationService.Session, userId: String, email: String) {
        // Save to UserDefaults (non-sensitive data)
        UserDefaults.standard.set(session.expiresAt, forKey: sessionKey)
        UserDefaults.standard.set(userId, forKey: userIdKey)
        UserDefaults.standard.set(email, forKey: userEmailKey)

        // Save sensitive tokens to Keychain
        saveToKeychain(key: "accessToken", value: session.accessToken)
        saveToKeychain(key: "refreshToken", value: session.refreshToken)
    }
    ...
    func isSessionValid() -> Bool {
        guard let expiresAt = UserDefaults.standard.value(forKey: sessionKey) as? Int else {
            return false
        }
    ...
    private func saveToKeychain(key: String, value: String) {
        ...
        let query: [String: Any] = [
            ...
            kSecAttrService as String: "com.neatlify.desktop.auth",
